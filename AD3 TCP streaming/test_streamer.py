...
class TcpStreamer:
    def __init__(
        ...
        host="0.0.0.0",
        port=9000,
        channel=0,               # Channel 1 in UI => API index 0
    ...
    def arm_device(self):
        ...
        check_ok(dwf.FDwfAnalogInChannelEnableSet(
            self.hdwf, ctypes.c_int(self.channel), ctypes.c_int(1)
        ))
        check_ok(dwf.FDwfAnalogInChannelRangeSet(
            self.hdwf, ctypes.c_int(self.channel), ctypes.c_double(self.input_range)
        ))
        check_ok(dwf.FDwfAnalogInChannelFilterSet(
            self.hdwf, ctypes.c_int(self.channel), ctypes.c_int(filterDecimate)
        ))
    ...
    def acquire_loop(self):
        c_samples = (ctypes.c_double * self.local_chunk)()
        ...
        while not self.stop_event.is_set():
            try:
                ...
                while n > 0 and not self.stop_event.is_set():
                    ...
                    arr64 = np.ctypeslib.as_array(c_samples)[:take]
                    arr32 = arr64.astype(np.float32, copy=True) * self.probe_scale
                    payload = arr32.tobytes(order="C")
    ...
    def send_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                self.samples_sent += len(payload) // 4
            except Exception as e:
